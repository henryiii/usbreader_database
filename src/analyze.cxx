...
int main(int argc, char *argv[]){
  ...
  for(pba = possible_board_address.begin(); pba!=possible_board_address.end(); ++pba){
    ...
    xxx.commit();
    //this used to convert HEX entry in DB to binary, removed as now storing binary in database...
    std::string jj = (std::string)((char*)r[0][0].c_str());
  ...
  TFile *output_root_file = new TFile("daq_root_file.root","RECREATE");
  TTree *output_root_tree = new TTree("daq_tree","daq_tree");

  int                 eID = 0;
  double              eTime;
  double              ePacketNumber;
  std::string         eBoardAddress;
  std::vector<int>    eChannels;
  std::vector<size_t> eChannelsHold;

  output_root_tree->Branch("eID",          &eID,          "eID/I");
  output_root_tree->Branch("eTime",        &eTime,        "eTime/D");
  output_root_tree->Branch("ePacketNumber",&ePacketNumber,"ePacketNumber/D");
  output_root_tree->Branch("eBoardAddress",&eBoardAddress);
  output_root_tree->Branch("eChannels",    &eChannels);
  ...
  while(true) {
	...
	//e.dump();
	eTime         = e.time();
	ePacketNumber = e.packet_number();
	eBoardAddress = e.board_address();
	eChannelsHold = e.channels();
	
	for ( auto const& ooo : eChannelsHold )
	  eChannels.push_back((int)(ooo));
	
	output_root_tree->Fill();
	eChannels.clear();
	eChannelsHold.clear();
      }
    }
    eID++;
    leveler.increase_the_time();
    bool fake = true;
    if(!fake) {
      ...
      xxx.commit();
      stored_packets[mindex] = Packet((std::string)((char*)r[0][0].c_str()));
      leveler.init(stored_packets, NUM_BOARDS);
      r.clear();
    }
