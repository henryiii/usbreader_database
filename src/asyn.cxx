...
int main(){
  ...
  // read  packet from each board
  std::clog << " qqq read 100 packets " << std::endl; fflush(stdout);
  std::vector<std::string> the_packets;
  for(int i = 0 ; i < 100 ; i++){ 
    bool oker = read_hex_from_board(cyppp[0], TIMEOUT, &local_packet);
    the_packets.push_back(local_packet);
    local_packet.clear();
  }
  std::clog << " qqq read  " << std::endl; fflush(stdout);
  ...
  std::cout << the_packets[0] << std::endl;
  size_t i = 0;
  for(std::vector<std::string>::iterator itr = the_packets.begin(); itr != the_packets.end(); ++itr){
    Packet *p = new Packet(db_hex_to_binary(512,(char*)(*itr).c_str()));
    p->packet_to_physical_parameters();
