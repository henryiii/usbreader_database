...
public:
  ...
  // constructor
  Event(std::vector<size_t> channels, double time, double packet_number, string board_address, size_t n_layers)
    {
      channels_ = channels;
      time_ = time/CLOCK_CYCLE_LENGTH;
      written_ = false;
...
ords " << endl;
  ...
  void Write(ofstream &data_file, size_t n_layers){
    ...
    data_file << (size_t)(packet_number() + 0.5) << " ";
    if( type_.compare("SLOW") == 0 )
      data_file << "1" << " ";
    else
      data_file << "0" << " ";
